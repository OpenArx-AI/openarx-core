...
describe('allPrimitives', () => {
  it('registers all 34 primitives with no id/version collision', () => {
    const reg = new Registry();
    reg.registerAll(allPrimitives(primDeps));
    const passports = reg.list();
    expect(passports).toHaveLength(34);
  ...
  it('covers all 5 categories with the expected counts', () => {
    ...
    // algorithmic 13 (+ route-intent §3.1, + derive-run-status §12.1) · retrieval 7 (+ fetch-run-path, + fetch-run-closeout §12.1) · state 9 · model-call 1
    expect(counts).toEqual({ transform: 4, algorithmic: 13, retrieval: 7, state: 9, 'model-call': 1 });
  });
});
