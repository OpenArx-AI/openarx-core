...
const langId: LangId = () => ({ lang: 'en', confidence: 1 });
const embed: Embed = () => [0];
...
describe('allPrimitives', () => {
  it('registers all 33 primitives with no id/version collision', () => {
    const reg = new Registry();
    reg.registerAll(allPrimitives(primDeps));
    const passports = reg.list();
    expect(passports).toHaveLength(33);
  ...
  it('covers all 5 categories with the expected counts', () => {
    ...
    // algorithmic 13 (+ route-intent §3.1, + derive-run-status §12.1) · retrieval 6 (+ fetch-run-path) · state 9 · model-call 1
    expect(counts).toEqual({ transform: 4, algorithmic: 13, retrieval: 6, state: 9, 'model-call': 1 });
  });
});
